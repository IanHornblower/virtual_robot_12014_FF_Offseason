...
public class SocketLinkedTeleOp extends LinearOpMode {
    @Override
    public void runOpMode() throws InterruptedException, IOException {
        ...
        SocketServer s = new SocketServer("localhost", 6969);
        ...
        new Thread(()-> {
            try {
                while(opModeIsActive()) {
                    s.sendValue(robot.localizer.getRotatedVelocityPos().heading);
                }
            } catch (IOException e) {
                e.printStackTrace();
            }
